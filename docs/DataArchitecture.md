...
2. **作为 REL 表达式求值的数据载体** — `Measurement` 和 `DataArray` 均可作为 REL 表达式的计算结果，支持带单位算术、多维索引切片和广播运算。REL 运行时用统一的 **`rel::Value`** 包装这两种类型：`Value` 对外提供统一的元数据与数据访问接口（`data_kind()` / `data_type()` / `data_shape()` / `unit()` / `rows()` / `indep_names()` / `dimension_spec()` / `data()` / `indep()` 等，见 [REL.md](../REL/REL.md) 数据类型章节），内部以 Measurement（按值）或 `shared_ptr<DataArray>` 存储，运算符与函数无需区分二者即可工作。

> **职责边界**：xdataset 提供**数据模型**（类型体系、维度、选择、DataFrame 展示）。
> REL 运行时以统一的 **`rel::Value`** 包装 `Measurement` / `DataArray` 并提供统一接口；
> 运算符（`+ - * / ...`）与数学/归约等**运算语义**实现在 REL 运行时层
> （`rel::Value` 运算符 → `rel::operation` 管线，见[运算（Arithmetic）](#运算arithmetic)），
> xdataset 本身不实现算术内核。本文档与 [REL.md](../REL/REL.md) 配合阅读。
...
package "xdataset" #FAFAFA {
    ...
    class Dataset {
        ...
        + GetDataArray(name)
        + HasUniqueDataArray(name) / IsLeaf(path)
    ...
    class DataFrame {
        ...
        + {static} FromDataArray(variable, name)
        + {static} FromMeasurement(measurement, name)
        + headers() / row_count()
        + GetRow(row)
        + SetVariableName(name)
        + ToCsv() / WriteToCsv(path)
        + to_string(max_rows)
    }
...
| DataType | `Real` / `Integer` / `Complex` / `String` / `Boolean` | 元素类别 |
| Shape | `[]` / `[width]` / `[rows, cols]` | 具体尺寸 |
| Unit | `Unit` | 物理单位 |

`DataType` x `DataKind` 共 5 x 3 = 15 种组合，涵盖标量、向量和矩阵形式的所有基本数据类型。
...
- `String` — 字符串 (`string`)
- `Boolean` — 逻辑真/假（REL 的比较与逻辑运算结果，按 Integer 0/1 参与算术提升）
...
`DataFrame` 是面向展示的只读表格抽象，将 Block / DataArray / Measurement 的数据展开为行列视图。

三种具体帧类型（BlockDataFrame / DataArrayDataFrame / MeasurementDataFrame）是库**内部实现**，
对外统一通过静态工厂创建，返回 `std::unique_ptr<DataFrame>`，调用方只使用基类接口：

```cpp
std::unique_ptr<DataFrame> f1 = DataFrame::FromBlock(block);                          // Block 宽表
std::unique_ptr<DataFrame> f2 = DataFrame::FromDataArray(da, "Vout");                 // 单变量 + 坐标轴
std::unique_ptr<DataFrame> f3 = DataFrame::FromMeasurement(m, "S(1,1)");             // 单行
```

便捷入口（内部懒加载缓存）：`Block::GetOrCreateDataFrame()`、
`DataArray::GetOrCreateDataFrame(variable_name = "data")`、`Measurement::to_dataframe(name)`
（返回 `std::unique_ptr<DataFrame>`，用 `->` 访问）。

基类接口：`headers()`、`row_count()`、`GetRow(row)`、`SetVariableName(name)`（设置依赖列标题，同名 no-op，仅刷新表头不重建行数据）、
`ToCsv()` / `WriteToCsv(path)`、`to_string(max_display_rows = 32)`（ASCII 表格展示，超出行数时省略）。
...
- **可变表头** — 通过 `SetVariableName(name)` 可设置列标题而不重建行数据（同名 no-op）；`GetOrCreateDataFrame(name)` 在 name 变化时自动调用
...
BlockDataFrame 和 DataArrayDataFrame 按固定行数分块生成，默认每块 128 行。访问某行时，仅当该行所在块尚未生成时才触发该块的批量计算并缓存。`ToCsv()` 遍历时同样按需触发，海量数据下仅在访问范围内分配内存。

## 运算（Arithmetic）

> **实现位置**：运算内核实现在 **REL 运行时**（`rel::Value` 运算符 → `rel::operation::Operate`
> 管线，`src/runtime/operation/`），不在 xdataset 内部。xdataset 的 `Measurement` / `DataArray`
> 是运算的输入与结果载体。本节描述 REL 表达式中一元 / 二元 / 三元运算的完整语义。

### 管线结构（REL runtime）

每个运算符对应一份 `OpTraits`（`arity`、`name`、四个 derive 回调与一个 execute 回调）。
`Operate(operands, traits)` 依次执行：

1. **形状推导**（`DeriveShapeBroadcast` 等）— 得到结果 DataShape 并构建形状广播计划
2. **行数推导**（`DeriveRowsBroadcast`）— 行数相等或 1 行广播，否则报错
3. **类型推导**（`DeriveDtypePromote` 等）— 见下文 DataType 推导
4. **单位推导**（`DeriveUnitXxx`）— 见下文 Unit 推导
5. **执行** — 扁平缓冲逐元素循环，结果包装为 `Value`

运算符形式（`+ - * / % ** == < <= > >= && || & | ^ << >>` 及一元 `- ! ~`）
与函数形式（`add`、`subtract`、`times`、`rdivide`、`pow`、`equal`、`conditional` …，
见 [REL.md](../REL/REL.md) 内建函数参考）共享同一套 `OpTraits`。

### 运算符类别

| 类别 | 运算符 / 函数 | 结果 DataType | 结果 Unit | 说明 |
|------|--------------|--------------|----------|------|
| kOpAdd / kOpSub | `+` `-` / `add` `subtract` | 按提升规则 | 双方同量纲取任一方，一方无量纲取另一方 | -- |
| kOpMul | `*` / `multiply` | 按提升规则 | 量纲相乘 | 矩阵乘法 A×B；含 Scalar 时退化为逐元素广播 |
| kOpDiv | `/` / `divide` | 按提升规则，int/int → Real | 量纲相除 | 矩阵语义 A×inv(B)；含 Scalar 时逐元素广播 |
| kOpTimes | `times` | 按提升规则 | 量纲相乘 | **纯逐元素**乘法（无矩阵语义） |
| kOpRdivide | `rdivide` | 至少 Real | 量纲相除 | **纯逐元素**除法 |
| kOpMod | `%` / `mod` | 按提升规则，不支持 Complex | 继承左操作数 | Real 用 `fmod` |
| kOpCompare | `==` `!=` `<` `>` `<=` `>=` / `equal` … | Boolean | 无量纲 | 要求双方量纲一致；复数按模比较 |
| kOpLogical | `&&` `\|\|` `!` / `and` `or` `not` | Boolean | 无量纲 | 非零为真 |
| kOpBitwise | `&` `\|` `^` `~` / `bitand` … | Integer | 无量纲 | 仅 Integer 操作数 |
| kOpShift | `<<` `>>` / `shiftleft` `shiftright` | Integer | 继承左操作数 | 仅 Integer 操作数 |
| kOpPow | `**` / `pow` | 至少 Real（`std::pow` 语义） | 继承底数 | 指数必须无量纲 |
| kOpConditional | `?:` / `conditional` | 条件 / 真值 / 假值共同推导 | 由分支决定 | 逐元素三选一 |
| kOpIf | `if…then…elseif…else` | 各分支共同推导 | 由分支决定 | 逐元素多分支选择 |

> 一元 `-`（取负）保持操作数类型和单位；`!`（逻辑非）与 `~`（按位取反）分别返回 Boolean 与 Integer，无量纲。
...
- **算术类（kAddSub / kMul / kDiv / kTimes / kRdivide / kMod / kPow）** — 按提升规则: Integer → Real → Complex
- **kDiv / kRdivide** — 同提升规则，但 `Integer / Integer` 强制提升为 `Real`
- **kCompare / kLogical** — 结果始终为 `Boolean`；**kBitwise / kShift** — 结果始终为 `Integer`
- **kPow** — 同提升规则，但至少 Real（`std::pow` 语义）
- **kMod** — 提升但排除 Complex

提升规则矩阵（`+` 行 × `+` 列）：

| × | Integer | Real | Complex | String | Boolean |
|---|---|---|---|---|---|
| **Integer** | Integer | Real | Complex | -- | Integer |
| **Real** | Real | Real | Complex | -- | Real |
| **Complex** | Complex | Complex | Complex | -- | Complex |
| **String** | -- | -- | -- | -- | -- |
| **Boolean** | Integer | Real | Complex | -- | Boolean |
...
当 `DataArray op DataArray` 时，逐行应用上表；当 `DataArray op Measurement` 时，`Measurement` 先广播到所有行，再逐元素应用上表。`times` / `rdivide` 与该矩阵一致；`multiply` / `divide`（矩阵语义）要求操作数为 Scalar（退化为逐元素）或 Matrix（行列匹配）。
...
1. **同量纲保留** — 与第一个条目量纲相同的条目保留其单位（如 `GHz` 与 `MHz` 同类）
2. **无量纲条目打标** — 无量纲条目被标记为当前目标单位（如无单位 `12.0` 与 `GHz` 拼接时被当作 `12.0 Hz`）；第一个无量纲条目不改变目标单位
...
将 `expr_list` 中每一项视为若干行数据，**逐行纵向拼接**为一个新的 Independent DataArray：

- **行数规则**：不同 item 的行数可以不等（各行拼接，总行数为各项行数之和）；每行的 shape 必须一致
- **结果类型**：始终为 DataArray（Independent），无上游坐标轴
- **DataType / Unit**：与 Matrix 相同的规则
